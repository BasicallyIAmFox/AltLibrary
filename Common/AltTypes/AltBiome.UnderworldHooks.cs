...
[LoadableContent(ContentOrder.Content, nameof(StaticLoad_UnderworldLighting))]
public abstract partial class AltBiome<T> {
	private static void StaticLoad_UnderworldLighting() {
	...
	IL_0031: stloc.3
			*/
			c.GotoNext(i => i.MatchStloc(out intensityIndex));
		...
		IL_0175: stloc.2
				*/
				c.GotoNext(MoveType.After,
					...
					i => i.MatchLdcR4(out _),
					i => i.MatchMul(),
					i => i.MatchLdloc(gIndex),

					i => i.MatchLdloc(intensityIndex),
					i => i.MatchLdcR4(out _),
					i => i.MatchMul(),
					i => i.MatchLdloc(bIndex));
				...
				c.Emit(OpCodes.Call, typeof(IAltBiome).FindMethod(nameof(ModifyUnderworldLighting)));
			};
